...
const EditAccomplishment = (props) => {
  ...
  const fetchAccomplishment = async () => {
    if (props.editId !== null) {
      const accom = await getAccomplishment(props.editId);
      if (accom.success) {
        var accomData = accom.data.data;
        setDate(new Date(accomData.attributes.date));
        coopname.current.value = accomData.attributes.coop_name;
        purpose.current.value = accomData.attributes.purpose;
        product.current.value = accomData.attributes.product;
        remarks.current.value = accomData.attributes.remarks;
      }
    }
  };

  const deleteAccom = async () => {
    const delAccom = await deleteAcomplishment(props.editId);
    if (delAccom.success) {
      setMsgColor("green");
      setSuccessMessage("Accomplishment deleted successfully");
      setDate(null);
  ...
  const handleSubmit = async (event) => {
    ...
    if (date !== null) {
      ...
      const editAccom = await updateAccomplishment(props.editId, data);
      if (editAccom.success) {
        setMsgColor("green");
        setSuccessMessage("Accomplishment edited successfully");
        setDate(null);
        coopname.current.value = "";
        product.current.value = "";
        purpose.current.value = "";
